 ...
 * fenced off at the top of the file — in practice only its `thisPackage` field,
 * because `dependencyGraph` below is the whole roster and is identical in every
 * copy. Everything below the fence is generic and should be copied byte-for-byte.
 *
 * ---------------------------------------------------------------------------
 * What it enforces
 * ---------------------------------------------------------------------------
 ...
 *   roster (plan.md §2.1), so every runtime cycle in the organisation is
 *   detectable from here — but only as long as the mirrored rows stay in sync
 *   with the sibling repositories they describe. That sync is manual today; the
 *   graph should eventually be published from `mc-dev-meta` and consumed.
 * - The graph models RUNTIME edges only. devDependency edges (mx-gameplay and
 *   mx-redstone on mc-playground-kit) are intentionally not nodes-to-nodes
 *   here: they cannot create a runtime cycle, and modelling them would make the
 *   real graph look cyclic (kit -> render -> sim, gameplay -> sim, and
 *   gameplay -.-> kit).
 */
 ...
 *
 * `dependencyGraph`
 *   Direct RUNTIME dependency edges, keyed by package name. The row for
 *   `thisPackage` is authoritative and is what the import check enforces. Rows
 *   for other packages are mirrored from their own repositories and exist so
 *   that cycle detection has something to work with; keeping them in sync is a
 *   manual chore today, and the organisation-wide graph should eventually be
 *   published from a single meta-repository (`mc-dev-meta`) and consumed here.
 *
 *   Two things are deliberately absent from every row:
 *
 *   - `@nerima-games/mc-kernel`. It is universally importable (rule 4), so
 *     listing it would be noise, and `checkPolicyConfiguration` rejects it
 *     outright to keep that true. Kernel is the ONLY such exception: every
 *     other edge must be declared, and no edge grants transitive rights.
 *   - `@nerima-games/mc-playground-kit` as a target. mx-gameplay and
 *     mx-redstone use it, but only as a devDependency (plan.md §2.3-2), and a
 *     devDependency is not a runtime edge — it cannot participate in a runtime
 *     cycle and must not be modelled as one. `DEV_ONLY_PACKAGES` + rule 6 is
 *     what enforces it. The kit itself DOES get a row, because its own
 *     dependencies (render/sim/worldgen) are real runtime edges.
 *
 * `aliases`
 ...
 *
 * ---------------------------------------------------------------------------
 * For mc-physics the allowed set is EMPTY, which reads as "kernel and nothing
 * else". Physics asks the world exactly one question — "is this cell solid, and
 * what shape does it collide with?" — and receives it as an injected callback,
 * so it depends on no world, no chunk manager and no renderer.
 *
 * That injection is also what keeps plan.md 3.4's "no block-ID name-checks"
 * rule enforceable: the caller resolves capability flags, physics only ever
 * sees booleans and shapes.
 *
 * NOTE ON THE SKELETON: at this stage nothing in the organisation is published
 * (the model is bottom-up publish-then-pin), so package.json declares `effect`
 * only and the shipped source imports no org package at all. The graph below is
 * still the full 16-repository roster, because the graph is the specification —
 * it is what makes cycle detection meaningful before the first publish.
 * ---------------------------------------------------------------------------
 */
export const REPOSITORY_POLICY = {
  thisPackage: '@nerima-games/mc-physics',

  /**
   * THE FULL 16-REPOSITORY GRAPH (plan.md 2.1), recorded in every copy.
   *
   * `@nerima-games/mc-kernel` is deliberately ABSENT from every row even though
   * plan.md draws an edge to it from almost every node. Kernel is universally
   * importable (rule 4) and `checkPolicyConfiguration` rejects a row that names
   * it explicitly; writing the kernel edges here would fail the gate rather
   * than document it. Read every row below as "... plus mc-kernel".
   *
   * Rows other than `thisPackage` exist so that cycle detection has a complete
   * graph to work with. They are mirrored by hand today; the organisation-wide
   * graph should eventually be published from mc-dev-meta and consumed here.
   *
   * `mc-playground-kit` appears as a NODE (it has runtime dependencies of its
   * own) but never as a TARGET: every experience module takes it as a
   * devDependency only, which creates no runtime edge and therefore cannot
   * participate in a cycle. `DEV_ONLY_PACKAGES` is what enforces that.
   */
  dependencyGraph: new Map<string, ReadonlySet<string>>([
    // -- tier 1: stable libraries. Mutually independent, kernel-only. --------
    ...
    // -- tier 2: foundations. State and services (nouns). --------------------
    ...
    // -- tier 3: experience modules. Rules and UI (verbs). -------------------
    // Zero edges between them: "mining puts an item in the inventory" travels
    // through mc-sim's InventoryService, not through a sibling import.
    // Each also takes mc-playground-kit as a devDependency, which is not an
    // edge and is therefore not written here.
    [
    ...
    // -- tier 4: composition. Layer merge + the stage total-order table. -----
    [
    ...
    // -- the dev workspace. Clones the other 15; no runtime dependency. ------
